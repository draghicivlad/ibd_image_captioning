...
class Flickr30k(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.processor:
            encoding = self.processor(images=image, text=caption, padding="max_length", return_tensors="pt", legacy=False)

            encoding = {k:v.squeeze() for k,v in encoding.items()}

            return encoding['pixel_values'], encoding['input_ids'], encoding['attention_mask'], torch.ones(len(encoding['input_ids']))
